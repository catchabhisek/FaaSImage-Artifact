...
func Unpack(decompressedArchive io.Reader, dest string, options *TarOptions) error {
	...
	// var timeElapsed = time.Since(start)
	// fmt.Fprintln(os.Stdout, "UNTAR: Unpack(): setup for unpacking the layer took %s", timeElapsed)

	var copy_time float64 = 0.0
	var whiteout_time float64 = 0.0
	var read_tar_time float64 = 0.0
...
loop:
	for {
		...
		read_tar_time += float64(time.Since(start).Nanoseconds())
		start = time.Now()
		...
		whiteout_time += float64(time.Since(start).Nanoseconds())

		start = time.Now()
		if err := createTarFile(path, dest, hdr, trBuf, !options.NoLchown, options.ChownOpts, options.InUserNS); err != nil {
			return err
		}
		copy_time += float64(time.Since(start).Nanoseconds())
	...
	fmt.Fprintln(os.Stdout, "UNTAR: Unpack(): The break is read tar file, copy file, whiteout file: ", read_tar_time, copy_time, whiteout_time)
