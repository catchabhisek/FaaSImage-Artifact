...
func (ls *layerStore) applyTar(tx *fileMetadataTransaction, ts io.Reader, parent string, layer *roLayer) error {
	...
	// logrus.Debugf("PULL_STATS: applyTar(): tarsplit writer and digester took %s and tarsplit is %s", timeElapsed, ls.useTarSplit)
	...
	// timeElapsed = time.Since(start)
	// logrus.Debugf("PULL_STATS: applyTar(): apply diff took %s", timeElapsed)
	// logrus.Debugf("PULL: The driver utilized by the code is %s", reflect.TypeOf(ls.driver))
...
func (ls *layerStore) registerWithDescriptor(ts io.Reader, parent ChainID, descriptor distribution.Descriptor) (Layer, error) {
	...
	// timeElapsed1 := time.Since(start1)
	// logrus.Debugf("PULL_STATS: APPLY_TAR: Driver creation took: %s", timeElapsed1)
	...
	// timeElapsed1 = time.Since(start1)
	// logrus.Debugf("PULL_STATS: APPLY_TAR: Transaction took: %s", timeElapsed1)
	...
	// timeElapsed1 = time.Since(start1)
	// logrus.Debugf("PULL_STATS: APPLY_TAR: apply_tar operation took: %s", timeElapsed1)
	...
	// timeElapsed1 = time.Since(start1)
	// logrus.Debugf("PULL_STATS: APPLY_TAR: storeLayer operation took: %s", timeElapsed1)
