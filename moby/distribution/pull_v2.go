...
`
	...
	// Question: Until now it has pulled the manifest.json file.
...
func ParsePackages(bench string) ([]*Package, error) {
	// Read the file
	fileData, err := ioutil.ReadFile(fmt.Sprintf("/home/user/FaaSSnapper/analysis/faasimage/data/input/%s.json", bench))
	if err != nil {
	...
	extractDir := "/var/lib/docker/packages"
	// extractDir := "/home/user/docker/packages"
...
func (p *puller) pullSchema2Layers(ctx context.Context, target distribution.Descriptor, layers []distribution.Descriptor, platform *specs.Platform, ref reference.Named) (id digest.Digest, err error) {
	...
	// Pull the packages from a remote repository
	go func() {
		if strings.Contains(ref.Name(), "_faas") {
			parts := strings.Split(ref.Name(), "/")
			benchmark := strings.TrimSuffix(parts[len(parts)-1], "_faas")
			logrus.Debugf("FAASIMAGE: The value %s and result %s", ref.Name(), benchmark)
			...
			start := time.Now()
			resp, err := http.Post("http://10.237.22.199:8080/upload", "application/json", bytes.NewBuffer(packagesJSON))
			if err != nil {
				fmt.Println("Error:", err)
				return
			}
			...
			timeElapsed := time.Since(start)
			logrus.Debugf("PULL_STATS: The download of the layer:packages took: %s", timeElapsed)
			...
			pgzipReader, err := pgzip.NewReader(tarBuffer)
			defer pgzipReader.Close()

			// Create a tar reader
			tr := tar.NewReader(pgzipReader)
			...
			for {
				...
				if header.Typeflag == tar.TypeSymlink {
					...
					err := os.Chtimes(extractPath, header.ModTime, header.ModTime)
					if err != nil {
						fmt.Println("Error changing modification time:", err)
						return
					}
					// Change the modification time of the directory
					err = os.Chtimes(dir, header.ModTime, header.ModTime)
					if err != nil {
						fmt.Println("Error changing modification time:", err)
						return
					}
				...
				fileInfo, err := os.Stat(extractPath)
				if os.IsNotExist(err) || (fileInfo.Size() == 32) {
					// Create extracted file
					extractFile, err := os.Create(extractPath)
					if err != nil {
						fmt.Println("Error creating file:", err, extractPath)
						return
					}
					...
					if _, err := io.Copy(extractFile, tr); err != nil {
						fmt.Println("Error writing file:", err, extractPath)
						return
					}
					...
					err = os.Chtimes(extractPath, header.ModTime, header.ModTime)
					if err != nil {
						fmt.Println("Error changing modification time:", err, extractPath)
						return
					}
					...
					err = os.Chtimes(dir, header.ModTime, header.ModTime)
					if err != nil {
						fmt.Println("Error changing modification time:", err, dir)
						return
					}
				}
			}
			...
			timeElapsed = time.Since(start)
			logrus.Debugf("PULL_STATS: The apply tar operation of the layer:packages took: %s", timeElapsed)
		}
	...
	logrus.Debugf("PULL: The pullSchema2Layers downloadedRootFS took")
...
func (p *puller) pullSchema2(ctx context.Context, ref reference.Named, mfst *schema2.DeserializedManifest, platform *specs.Platform) (id digest.Digest, manifestDigest digest.Digest, err error) {
	...
	logrus.Debugf("FAASIMAGE: I have got the name %s", ref)
	id, err = p.pullSchema2Layers(ctx, mfst.Target(), mfst.Layers, platform, ref)
