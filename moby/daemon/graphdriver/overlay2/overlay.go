...
func (d *Driver) Get(id, mountLabel string) (_ containerfs.ContainerFS, retErr error) {
	...
	absLowers := make([]string, len(splitLowers)+1)
	// absLowers[0] = "/home/user/docker/packages"
	absLowers[0] = "/var/lib/docker/packages"
	logrus.Debugf("I am here!!!")
	for i, s := range splitLowers {
		absLowers[i+1] = path.Join(d.home, s)
		logrus.Debugf("I am here!!!")
	}
	logrus.Debugf("I am here!!!")
	/*
	...
	// logrus.Debugf("DEPLOY: overlayfs  got the values mountTarget %s, mountData %s as", mountTarget, mountData)
...
func (d *Driver) ApplyDiff(id string, parent string, diff io.Reader) (size int64, err error) {
	logrus.Debugf("STORAGE: The applydiff of overlay2")
	...
	// logrus.Debugf("PULL_STATS: applyTar(): untar operation took %s", timeElapsed)
