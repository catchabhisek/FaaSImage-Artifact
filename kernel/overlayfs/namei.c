...
static void print_file_path(struct dentry *dentry, const char *name) {
	char *tmp, *p = kmalloc(4096, GFP_HIGHUSER_MOVABLE);
	pid_t pid = current->pid;
	if (p) {
 		tmp = dentry_path_raw(dentry, p, 4096);
 		printk(KERN_INFO "PID: %d is accessing file %s/%s\n", pid, tmp, name);
 		kfree(p);
 	}
}

// static void print_file_path(struct dentry *dentry) {
// 	char *tmp, *p = kmalloc(4096, GFP_HIGHUSER_MOVABLE);
// 	pid_t pid = current->pid;
// 	if (p) {
// 		tmp = dentry_path_raw(dentry, p, 4096);
// 		printk(KERN_INFO "PID: %d is accessing file %s\n", pid, tmp);
// 		// printk(KERN_INFO "PID: The mount point information is %s\n", dentry->d_name.name);
// 		kfree(p);
// 	}
// }
...
/*
type: 0: package.
type: 2: func pointers.
type: 3: base/apt.
*/
static int get_type(char *file_path, int max_len) {
	char package_path[] = "/package_repo";
	char func_bench_path[] = "/bench";
	char func_pycache_path2[] = "/__pycache__";
	char func_package_path[] = "/usr/local/lib/python3.10/site-packages";
	char base_package_path1[] = "/usr/local/lib/python3.10/site-packages/_distutils_hack";
	char base_package_path2[] = "/usr/local/lib/python3.10/site-packages/pkg_resources";
	char base_package_path3[] = "/usr/local/lib/python3.10/site-packages/distutils-precedence.pth";
	char base_package_path4[] = "/usr/local/lib/python3.10/site-packages/pip";
	char base_package_path5[] = "/usr/local/lib/python3.10/site-packages/setuptools";
	char base_package_path6[] = "/usr/local/lib/python3.10/site-packages/wheel";
	char base_package_path7[] = "/usr/local/lib/python3.10/site-packages/README.txt";
	char shared_lib_path[] = "/usr/lib/x86_64-linux-gnu";

	if(strstr(file_path, shared_lib_path))
		return -1;
	else if (strstr(file_path, func_pycache_path2))
		return 2;
	else if (strstr(file_path, package_path))
		return 0;
	else if (strstr(file_path, base_package_path1) || strstr(file_path, base_package_path2) || strstr(file_path, base_package_path3) || strstr(file_path, base_package_path4) || strstr(file_path, base_package_path5) || strstr(file_path, base_package_path6) || strstr(file_path, base_package_path7))
		return max_len-1;
	if (strstr(file_path, func_package_path) || (strstr(file_path, func_bench_path)))
		return 2;
	else
		return max_len-1;
}
...
// /*
// Layers: [package, container-init, function_pointer, apt-layer, base]
// */
// static void validate_layer_dentry(struct ovl_entry *entries, struct dentry *required, struct dentry *found) {
// 	char *tmp1, *tmp2, *tmp3;
// 	char *input = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN_POISON);
// 	char *predicted = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN_POISON);
// 	char *original = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN_POISON);

// 	if (input) {
// 		tmp1 = dentry_path_raw(required, input, 4096);
// 	} else {
// 		printk(KERN_INFO "The input variable is empty\n");
// 	}

// 	int type = get_type(tmp1, entries->numlower);

// 	if(original) {
// 		tmp2 = dentry_path_raw(found, original, 4096);
// 	} else {
// 		printk(KERN_INFO "The original variable is empty\n");
// 	}

// 	if(type < entries->numlower) {
// 		if(predicted) {
// 			tmp3 = dentry_path_raw(entries->lowerstack[type].dentry, predicted, 4096);
// 		} else {
// 			printk(KERN_INFO "The predicted variable is empty\n");
// 		}

// 		if(strstr(tmp2, tmp3))
// 		{
// 			printk(KERN_INFO "SUCCESSFULLY PREDICTION %s:%s\n", tmp3, tmp2);
// 		} else
// 		{
// 			printk(KERN_INFO "WRONG PREDICTION FOR %s:%s\n", tmp3, tmp2);
// 		}
// 	}
...
static int contains_dentry(struct dentry *dentry1, struct dentry *dentry2) {
	int result=0;
	char *tmp1, *tmp2;
	char *input1 = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN);
	char *input2 = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN);

	if(input1 && input2) {
		tmp1 = dentry_path_raw(dentry1, input1, 4096);
		tmp2 = dentry_path_raw(dentry2, input2, 4096);
	} else {
		printk(KERN_INFO "The input variable is failed allocation\n");
	}

	if(strstr(tmp1, tmp2)) {
		// printk(KERN_INFO "The input variable is matching: %s:%s\n", tmp1, tmp2);
		result=1;
	}
	// else {
	// 	printk(KERN_INFO "The input variable is not matching: %s:%s\n", tmp1, tmp2);
	// }
			  ...
			  unsigned int flags)
{
	...
	int attempt = 0;
	char *tmp1, *tmp2;
	char *input = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN);
	int predict_dir  = -100;  // I have not found anything yet.
	int predict_file = -100;  // I have not found anything yet.
	...
	// FAASIMAGE START
	if (input) {
		tmp1 = dentry_path_raw(dentry, input, 4096);
		type = get_type(tmp1, ovl_numlower(roe));
	}
	kfree(input);

	// printk(KERN_INFO "type is %d, row number is %d, poe number is %d\n", type, ovl_numlower(roe), ovl_numlower(poe));

	// for (i = 0; i < ovl_numlower(poe); i++) {
	// 	print_file_path(ovl_lowerstack(poe)[i].dentry);
	// }

	// for (i = 0; i < ovl_numlower(roe); i++) {
	// 	print_file_path(ovl_lowerstack(roe)[i].dentry);
	// }

repredict:
	struct dentry *predicted_dentry = NULL;
	struct ovl_lookup_data e = {
		.sb = dentry->d_sb,
		.name = dentry->d_name,
		.is_dir = false,
		.opaque = false,
	...
	if((type < ovl_numlower(roe)) || (type == -1)) {
		if(type == -1) {
			for (j=3; j>1; j--) {
				for (i = 0; i < poe->__numlower; i++) {
					struct ovl_path lower = poe->__lowerstack[i];
					if(contains_dentry(lower.dentry, roe->__lowerstack[j].dentry)) {
						e.mnt = lower.layer->mnt;
						err = ovl_lookup_layer(lower.dentry, &e, &predicted_dentry, false);
						if (err)
							goto out_put;
						...
						if ((e.stop)) {
							found=1;
							predict_file = i;
							if((i_size_read(d_inode(predicted_dentry)) == 32) && (attempt == 0)) {
								struct nlmsghdr *nlhead = NULL;
								struct sk_buff *skb_out = NULL;

								download_flag = 1;
								/* Send a broadcast message to the client that we need this file*/
								char *missing_file = kmalloc(4096, GFP_USER|__GFP_MOVABLE|__GFP_SKIP_KASAN);
								if (missing_file) {
									tmp2 = dentry_path_raw(predicted_dentry, missing_file, 4096);
								}

								// printk(KERN_INFO "The message sent is %s\n", tmp2);
								
								skb_out = nlmsg_new(strlen(tmp2), 0);  //nlmsg_new - Allocate a new netlink message: skb_out

								if(!skb_out)
								{
									printk(KERN_ERR "Failed to allocate new skb in overlayfs\n");
									goto out_put;
								}

								nlhead = nlmsg_put(skb_out, 0, 0, NLMSG_DONE, strlen(tmp2), 0);   // Add a new netlink message to an skb

								strncpy(nlmsg_data(nlhead), tmp2, strlen(tmp2));
								nlmsg_multicast(nl_sk, skb_out, 0, 17, GFP_KERNEL);
			...
			// Let us mimick the the overlay code:
			for (i = 0; i < ovl_numlower(poe); i++) {
				struct ovl_path lower = ovl_lowerstack(poe)[i];
				if(contains_dentry(lower.dentry, ovl_lowerstack(roe)[type].dentry)) {
					e.mnt = lower.layer->mnt;
					err = ovl_lookup_layer(lower.dentry, &e, &predicted_dentry, false);
					if (err)
						goto out_put;
	...
	// if ((predict_dir == -100) && (predict_file == -100)) {
	// 	// print_file_path(dentry);
	// 	printk(KERN_INFO "SHIT NOT FOUND THE LAYER for type %d\n", type);
	// 	// printk(KERN_INFO "The parameters for e are: %d\n", e.is_dir);
	// 	// for (i = 0; i < poe->numlower; i++) {
	// 	// 	print_file_path(poe->lowerstack[i].dentry);
	// 	// }
	// }
	...
	for (i = 0; !d.stop && i < ovl_numlower(poe); i++) {
		...
		if((predict_dir == -100) && (predict_file == -100)) {
			continue;
			// printk(KERN_INFO "SEARCHING FOR THE LAYER with type %d\n", type);
		...
		// if((predict_dir != -100) && (predict_file == -100)) {
			...
			// 	goto out_put;
		// }
		else if (predict_file == i) {
			// printk(KERN_INFO "EUREKA");
			d=e;
			this=predicted_dentry;
		}
		else if (predict_dir == i) {
			// printk(KERN_INFO "EUREKA");
			d=e;
			this=predicted_dentry;
		}
		else if ((predict_dir != -100) && (predict_file == -100)) {
			// printk(KERN_INFO "EUREKA");
			d.mnt = lower.layer->mnt;
			err = ovl_lookup_layer(lower.dentry, &d, &this, false);
			if (err)
